...
export function orcaApiSwap(
  ...
  return new Promise<void>((resolve, reject) => {
    const swap = async () => {
      ...
      const fromOrcaPoolConfig =
        OrcaPoolConfig[`${orcaPairing}` as keyof typeof OrcaPoolConfig];
      ...
      try {
        ...
        const quote = await orcaPool.getQuote(
          orcaToken(side),
          orcaAmount,
          slippage
        );
        ...
        console.log(
          `Swap ${orcaAmount.toString()} ${
            orcaPool.getTokenA().name
          } for at least ${usdcAmount.toNumber()} usdc (${side})`
        );
